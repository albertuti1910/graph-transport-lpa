...
|---|---|
| Proyecto práctico usando computación en la nube | Repositorio completo + demo local y opción AWS descritas en [README.md](../README.md) |
| Programar en Python/Java/otro lenguaje | Implementación en **Python 3.12** (API + worker) en `src/` y tests en `tests/` |
| Uso de AWS o LocalStack como entorno simulado | LocalStack en `docker-compose.yml` (perfil `demo` para stack completo); AWS real vía Terraform en `infra/` y runtime en `docker-compose.aws.yml` |
...
| Demostración del sistema (funciona) | Demo web local (`docker-compose.demo.yml`) y deploy AWS (`scripts/aws_deploy.sh`) descritos en [README.md](../README.md) |
